...
v0.3 当前不扩重型 chart、独立 ChargingPlace 表或无真实需求的期间筛选。

Reliability follow-up 优先于继续堆统计 UI。
...
不删除、不回退现有 sync foundation，但暂停继续扩 Spring Boot / PostgreSQL / Trip cloud sync，直到 Trip P0 reliability 具备可观察性并完成一次锁屏长行程复验。

之后再继续：

1. Vehicle / ChargingRecord sync DTO / envelope
2. 纯 Kotlin conflict/apply rules
3. pull cursor / push acknowledgement
4. 最小 HTTPS sync client/server
...
- Build Run #169 Green
- 正式建立 v0.4 Local First Sync Foundation
