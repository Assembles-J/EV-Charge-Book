...
class MainActivity: ComponentActivity(){
 ...
 override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);setContent{MainApp(vm)}}
}
...
fun MainApp(viewModel:MainViewModel){
 ...
 MaterialTheme{
  Scaffold(bottomBar={NavigationBar{titles.forEachIndexed{i,t->NavigationBarItem(selected=tab==i,onClick={tab=i},icon={Icon(listOf(Icons.Default.Home,Icons.Default.History,Icons.Default.BarChart,Icons.Default.DirectionsCar)[i],t)},label={Text(t)})}}}){p->
   Surface(Modifier.padding(p)){
    if(editVehicle){
     ...
     1->RecordsScreen(state.chargingRecords,viewModel::deleteChargingRecord)
     2->StatsScreen(state)
