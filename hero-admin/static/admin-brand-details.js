(function(){
  ...
  const BRAND_ID_RE=/^[a-z0-9][a-z0-9-]{1,100}$/;
  ...
  function targetOptions(source){return (catalog?.brands||[]).filter(b=>b.brandId!==source.brandId).sort((a,b)=>Number(isLegacyId(a.brandId))-Number(isLegacyId(b.brandId))||(a.name||'').localeCompare(b.name||'','zh-CN')).map(b=>`<option value="${escapeHtml(b.brandId)}">${escapeHtml(b.name)} · ${escapeHtml(b.brandId)}${isLegacyId(b.brandId)?' · legacy?':''}</option>`).join('')+`<option value="__new__">+ 新建规范品牌并合并</option>`}
  ...
  function openBrandDetail(brand){
    ...
    root.innerHTML=`${warning}<div class="status"><strong>${escapeHtml(brand.name)}</strong> · ${escapeHtml(brand.englishName||'--')} · <code>${escapeHtml(brand.brandId)}</code> · ${brand.isActive===false?'已下架':'在用'}<br/>旗下车型 ${list.length} 个 · Light ${brand.logoLightUrl?`v${brand.logoLightVersion||0}`:'未发布'} · Dark ${brand.logoDarkUrl?`v${brand.logoDarkVersion||0}`:'未发布'}</div><div class="logo-variant-grid" style="margin-top:14px">${logoCard(brand,'light')}${logoCard(brand,'dark')}</div><h3 style="margin:22px 0 10px">旗下车型</h3>${vehicleRows(list)}<div class="status" style="margin-top:18px"><strong>合并历史品牌</strong><br/>如果这是随机/重复 brandId，可把旗下车型改指向一个规范品牌，再自动下架当前品牌。目标品牌现有 Logo 会继续复用；不会删除历史图片文件。<div class="action-group" style="justify-content:flex-start;margin-top:10px"><select id="mergeBrandTarget" style="min-width:280px">${targetOptions(brand)}</select><button id="mergeBrandNow" class="danger">确认合并并下架当前品牌</button></div><div id="newMergeBrandFields" class="hidden" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:10px"><input id="mergeNewBrandId" placeholder="规范 brandId，例如 xiaomi" /><input id="mergeNewBrandName" value="${escapeHtml(brand.name)}" placeholder="品牌名称" /><input id="mergeNewBrandEnglish" value="${escapeHtml(brand.englishName||'')}" placeholder="英文名称（可空）" /></div><div class="meta" id="mergeBrandStatus" style="margin-top:8px">如果没有规范目标品牌，可选择“新建规范品牌并合并”；brandId 必须手工确认，系统不会猜。</div></div>`;
    const target=document.getElementById('mergeBrandTarget'),newFields=document.getElementById('newMergeBrandFields');target.onchange=()=>newFields.classList.toggle('hidden',target.value!=='__new__');if(target.options.length===1){target.value='__new__';newFields.classList.remove('hidden')}
    document.getElementById('mergeBrandNow').onclick=()=>mergeBrand(brand);dialog.showModal();
  }
  ...
  async function mergeBrand(source){
    const targetId=document.getElementById('mergeBrandTarget').value,list=vehiclesFor(source.brandId),status=document.getElementById('mergeBrandStatus');let target=(catalog?.brands||[]).find(b=>b.brandId===targetId)||null,newTarget=false;
    if(targetId==='__new__'){
      const brandId=String(document.getElementById('mergeNewBrandId').value||'').trim().toLowerCase(),name=String(document.getElementById('mergeNewBrandName').value||'').trim(),englishName=String(document.getElementById('mergeNewBrandEnglish').value||'').trim();
      if(!BRAND_ID_RE.test(brandId)){status.textContent='请输入稳定的小写 ASCII brandId（仅 a-z / 0-9 / 连字符）。';status.style.color='#ffb5b5';return}
      if(brandId===source.brandId){status.textContent='新 brandId 不能与当前历史 brandId 相同。';status.style.color='#ffb5b5';return}
      if(!name){status.textContent='品牌名称不能为空。';status.style.color='#ffb5b5';return}
      target=(catalog?.brands||[]).find(b=>b.brandId===brandId)||{brandId,name,englishName,isActive:true};newTarget=!(catalog?.brands||[]).some(b=>b.brandId===brandId);
    }
    if(!target)return;
    const logoWarning=(source.logoLightUrl||source.logoDarkUrl)&&!(target.logoLightUrl||target.logoDarkUrl)?'\n\n注意：旧品牌有 Logo，而新目标目前无 Logo。车型迁移后请在目标品牌重新上传/确认 Logo；旧图片文件不会删除。':'';
    if(!confirm(`把 ${source.name} (${source.brandId}) 合并到 ${target.name} (${target.brandId})？\n\n将改写 ${list.length} 个车型的 brandId，然后下架旧品牌。目标品牌 Logo 将继续复用；不会删除 Hero、Logo 文件或用户历史。${logoWarning}`))return;
    try{
      if(newTarget){status.textContent=`正在创建规范品牌 ${target.brandId}…`;const created=await postJson('api/brand/save',{brandId:target.brandId,name:target.name,englishName:target.englishName});target=created.brand||target}
